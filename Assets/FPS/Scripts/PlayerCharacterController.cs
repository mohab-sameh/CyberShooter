...
public class PlayerCharacterController : MonoBehaviour
{
    ...
    private enum State {Normal, MidHook, Thrown}
    private State state;
    ...
    {
        if (TestInputDownHookShot() && isGrounded==false)
        {
    ...
    void HandleHookThrow()
    {
        hookShotTransform.LookAt(hookShotPos);

        float HookShotThrowSpeed = 85f;
        hookShootSize += HookShotThrowSpeed * Time.deltaTime;
        hookShotTransform.localScale = new Vector3(1, 1, hookShootSize);

        if (hookShootSize >= Vector3.Distance(transform.position, hookShotPos))
        {
            state = State.MidHook;
    ...
    {
        hookShotTransform.LookAt(hookShotPos);
        float maxHookSpeed = 40f;
        float minHookSpeed = 15f;
        float hookShootSpeed = Mathf.Clamp(Vector3.Distance(transform.position, hookShotPos), minHookSpeed, maxHookSpeed);
        float hookShootMultiplier = 2.5f;
        Vector3 hookShotDir = (hookShotPos - transform.position).normalized;
        float reachedHookPos = 2f;
        m_Controller.Move(hookShotDir * hookShootSpeed * hookShootMultiplier * Time.deltaTime);
        if(Vector3.Distance(hookShotPos,transform.position) < reachedHookPos)
        {
            state = State.Normal;
            StopHookShot();
        ...
        {
            float extraMomentum = 4f;
            characterVelocity = hookShotDir * hookShootSpeed * extraMomentum;
            state = State.Normal;
            StopHookShot();
